...
if [[ "${SKIP_ANCILLARY:-false}" != "true" ]]; then
  ...
  # Mode handling
  if [[ "$SCRIPT_MODE" == "now" ]]; then
    # Explicit for clarity (Python defaults to now anyway)
    CMD_ANC_DEF+=( --mode now )
  else
    # history mode
    CMD_ANC_DEF+=( --mode history )

    # If TIME_RUN has a time component, pass --max-time HH:MM to Python
    if [[ "$TIME_RUN" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}[[:space:]]([0-9]{2}:[0-9]{2})$ ]]; then
      MAX_TIME="${BASH_REMATCH[1]}"
      CMD_ANC_DEF+=( --max-time "$MAX_TIME" )
    fi
  fi
