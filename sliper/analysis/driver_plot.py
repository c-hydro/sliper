...
class DriverPlot:
    ...
    def view_ts(self,
                ...
                min_ts_event=0, max_ts_event=18, min_ts_level=0, max_ts_level=4,
                time_format='%m-%d %H',
        ...
        # get forecast first time index
        ts_sm_frc_mask = ts_data["sm_period_type"].isin(["Observed_Forecast", "Forecast"])
        ts_sm_frc_first_index = ts_data.index[ts_sm_frc_mask].min() if ts_sm_frc_mask.any() else None
        ts_rain_frc_mask = ts_data["rain_period_type"].isin(["Observed_Forecast", "Forecast"])
        ts_rain_frc_first_index = ts_data.index[ts_rain_frc_mask].min() if ts_rain_frc_mask.any() else None

        if ts_rain_frc_first_index is not None and ts_sm_frc_first_index is not None:
            if ts_rain_frc_first_index != ts_sm_frc_first_index:
                log_stream.warning(' ===> First time index forecast is not the same for rain and soil moisture dataset')
        ts_first_frc_index = ts_rain_frc_first_index
        ...
        [tick_time_period, tick_time_idx, tick_time_labels] = configure_time_series_axes(
            ts_data,  time_format=time_format)
        ...
        # compute first forecast index
        if ts_first_frc_index is not None:
            # if ts_first_frc_index is a timestamp, convert to numeric x index
            if isinstance(ts_first_frc_index, pd.Timestamp):
                # find the closest tick index
                ts_first_frc_index = tick_time_labels.index(ts_first_frc_index.strftime(time_format)) \
                    if ts_first_frc_index.strftime(time_format) in tick_time_labels else None
            else:
                ts_first_frc_index = None
        ...
        # plot rainfall bars and peak rain line
        ...
        # twin axis for soil moisture
        ax8 = ax1.twinx()
        p8_1 = ax8.plot(np.arange(len(tick_time_labels)), list(ts_sm.values[:, 0]),
                        label=label_sm, **style_sm)
        ...
        # add vertical line for first forecast index
        if ts_first_frc_index is not None:
            ax1.axvline(
                x=ts_first_frc_index,
                color="black",
                linestyle="--",
                linewidth=2.5,  # <-- line thickness
                alpha=0.9,
                label=f"Start Forecast ({tick_time_labels[ts_first_frc_index]})"
            )

            # optional annotation above the line
            ax1.text(
                ts_first_frc_index + 0.2, max_ts_rain * 0.9,
                "forecast",
                rotation=90,
                color="black",
                fontsize=9,
        ...
        ax3.set_ylim(min_ts_event, max_ts_event)
        ...
        if ts_first_frc_index is not None:
            ...
            # optional annotation above the line
            ax3.text(
                ts_first_frc_index + 0.2, max_ts_event * 0.9,
                "forecast",
                rotation=90,
                color="black",
                fontsize=9,
