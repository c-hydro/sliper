...
def filter_file_csv_by_time(
    ...
    """
    # Check if 'time' is a column or index and set it to index
    if 'time' in df.columns:
        df = df.set_index(datetime_col)
        log_stream.warning(" ===> 'time' was a column and is now set as the index.")
    elif 'time' in df.index.names:
        log_stream.warning(" ===> 'time' is already the index.")
    else:
        log_stream.warning(" ===> 'time' is neither a column nor an index.")
