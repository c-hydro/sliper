...
def compute_data_metrics(da_var, column_name=None, metrics=None):
    ...
    for metric in metrics:
        ...
        if metric == 'avg':
            ...
            results['first'] = (
                float(first_value[column_name].iloc[0]) if isinstance(first_value[column_name], pd.Series)
                else float(first_value[column_name])
            )
            ...
            results['last'] = (
                float(last_value[column_name].iloc[0]) if isinstance(last_value[column_name], pd.Series)
                else float(last_value[column_name])
            )
