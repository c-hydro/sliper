...
# -------------------------------------------------------------------------------------
# Method to read data
def read_data_tiff(file_name, var_scale_factor=1,
                   var_type='float32', var_name=None, var_time=None, var_no_data=-9999.0, var_proj='EPSG:4326',
    ...
    if os.path.exists(file_name):
        ...
        lon = np.linspace(center_left, center_right, file_wide)
        lat = np.flip(np.linspace(center_bottom, center_top, file_high))
        ...
        geo_attrs = {'nrows': var_geo_y_2d.shape[0], 'ncols': var_geo_x_2d.shape[1],
                     'epsg': epsg_string,
                     'height': file_high, 'width': file_wide,
