        ...
        for var_name in var_list:
            ...
            if var_time_period is not None:
                ...
                for time_step in var_time_period:
                    ...
                    # define time step field(s)
                    alg_template_step = dict.fromkeys(list(alg_template_time.keys()), time_step)
                    # define time run fields() -- if are in the filename string(s)
                    alg_template_step.update({
                        'run_datetime': self.time_reference,
                        'run_sub_path_time': self.time_reference
                    })
                    ...
                    for (var_time, var_path_list_anc), var_path_list_dst in zip(
                        ...
                        if os.path.exists(var_file_path_anc):
                            ...
                            if var_dset_anc:
                                ...
                                # mask dataset to the geographical reference
                                if method_mask == 'reference':
                                    var_dset_masked = var_dset_masked.where(geo_da_dst == 1)
