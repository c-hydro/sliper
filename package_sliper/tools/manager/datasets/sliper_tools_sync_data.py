...
def load_config(path: Path) -> Config:
    ...
    if time_ref not in {"now","history"}: print("Config: time_ref must be 'now' or 'history'",file=sys.stderr); sys.exit(4)
    tz=raw.get("tz","Europe/Rome"); history_date=raw.get("history_date")
    if time_ref=="history":
        if not history_date: print("Config: history_date required when time_ref == 'history'",file=sys.stderr); sys.exit(4)
        try: datetime.strptime(history_date,"%Y-%m-%d")
        except ValueError: print(f"Config: invalid history_date: {history_date}",file=sys.stderr); sys.exit(4)
    else: history_date=None
    ...
    if not isinstance(files_raw,dict) or not files_raw: print("Config: 'files' must be non-empty mapping",file=sys.stderr); sys.exit(4)
    ...
    if m is not None:
        try: m=float(m)
        except Exception: print("Config: args.max_diff_hours must be numeric",file=sys.stderr); sys.exit(4)
    return Config(time_ref=time_ref,tz=tz,history_date=history_date,
...
def expand_day(t:str,d:date)->str: return t.replace("{YYYY}",f"{d.year:04d}").replace("{MM}",f"{d.month:02d}").replace("{DD}",f"{d.day:02d}")
def to_path(t:str,d:date)->Path: return Path(expand_day(t,d))
def filename_glob_from_template(t:str,d:date)->str: return t.replace("{YYYY}",f"{d.year:04d}").replace("{MM}",f"{d.month:02d}").replace("{DD}",f"{d.day:02d}").replace("{RUN}","*")
def parse_run_from_filename(tmpl:str,name:str)->Optional[str]:
    p=re.escape(tmpl); p=p.replace(r"\{YYYY\}",r"(?P<YYYY>\d{4})").replace(r"\{MM\}",r"(?P<MM>\d{2})").replace(r"\{DD\}",r"(?P<DD>\d{2})").replace(r"\{RUN\}",r"(?P<RUN>\d{4})")
...
def select_newest_summary(cfg:Config,d:date,run_cutoff:Optional[int]=None)->Path:
    ...
    if not folder.exists(): print(f"Summary folder not found: {folder}",file=sys.stderr); sys.exit(3)
    glob_pat=filename_glob_from_template(cfg.summary.filename_template,d)
    candidates=sorted(folder.glob(glob_pat))
    if not candidates: print(f"No summary files in {folder} matching {glob_pat}",file=sys.stderr); sys.exit(2)
    ...
    if newest[0] is None: print("No valid RUN parsed from filenames",file=sys.stderr); sys.exit(2)
    return newest[0]
def yyyymmddhhmm_to_dt(s:str,tz:str)->datetime: return datetime.strptime(s,"%Y%m%d%H%M").replace(tzinfo=ZoneInfo(tz))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.when or (args.date and args.time):
        cfg.time_ref="history"; cfg.history_date = args.when.split("T")[0] if args.when else args.date
    ...
    if not (run_date and run): print("summary missing run_date/run",file=sys.stderr); return 4
    ...
    if not (date_ini and date_end): print("summary missing DateIni/DateEnd",file=sys.stderr); return 4

    limit_to_obs = bool(getattr(args,"limit_to_obs",False) or getattr(args,"end_at_date_obs",False))
    if limit_to_obs and not date_obs: print("summary missing DateObs with --limit-to-obs/--end-at-date-obs",file=sys.stderr); return 4
    ...
    if oldest_sf_str and isinstance(oldest_sf_str, str):
        try: oldest_dt = datetime.strptime(oldest_sf_str,"%Y-%m-%d %H:%M").replace(tzinfo=ZoneInfo(info_tz)).astimezone(ZoneInfo(tz))
        except Exception: oldest_dt = None
    ...
    for DOMAIN in domains:
        for group_name,spec in cfg.files.items():
            ...
            data_times_and_files: List[Tuple[datetime,Path]] = []
            for p in candidates:
                data_ts = dt_from_filename(p.name, fn_dt_regex, tz) or datetime.fromtimestamp(p.stat().st_mtime, tz=ZoneInfo(tz))
                if dt_ini <= data_ts <= dt_end:
                    data_times_and_files.append((data_ts,p))
            data_times_and_files.sort(key=lambda x:x[0])

            selected: List[Path] = []
            if args.freq_minutes:
                step=args.freq_minutes*60; last_kept: Optional[datetime]=None
                for t,p in data_times_and_files:
                    if last_kept is None or (t - last_kept).total_seconds() >= step:
                        selected.append(p); last_kept=t
            else:
                selected=[p for _,p in data_times_and_files]
            ...
            if cfg.args.max_diff_hours is None:
                ...
                gated=[]; sel_map = {p: t for t,p in data_times_and_files}
                for _, p in data_times_and_files:
                    if p not in selected: continue
                    file_mtime = datetime.fromtimestamp(p.stat().st_mtime, tz=ZoneInfo(tz))
            ...
            for p in gated:
                ...
                ops.append({"from":str(p), "to":str(target)})
                if not args.dry_run:
